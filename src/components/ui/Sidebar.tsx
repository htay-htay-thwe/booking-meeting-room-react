import { Menu, Home, User, ClipboardClock, BarChart3 } from "lucide-react";
...
export default function Sidebar({ isOpen, setIsOpen, userRole }: SidebarProps) {
  ...
  const navLinkClass = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-x-4 p-3 rounded-xl transition-all duration-200 ${
      isActive ? "bg-amber-500 text-white shadow-md" : "text-stone-400 hover:bg-stone-800 hover:text-white"
    }`;
